...
const cekjodoh = async (nama, pasangan) => new Promise((resolve, reject) => {
    axios.get(`https://api.dapuhy.xyz/api/fun/jodoh?nama1=${nama}&nama2=${pasangan}&apikey=${dapuhyapi}`)
        .then((res) => {
            const textc = `Nama: ${res.data.result.nama}\nPasangan: ${res.data.result.pasangan}\nPositif: ${res.data.result.positif}\nSisi Negatif: ${res.data.result.negatif}`
            resolve({ text: textc })
        })
        .catch((err) => {
            reject(err)
