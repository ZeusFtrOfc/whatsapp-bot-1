     ...
     if (chats == '#rhentai'){
      ...
      if (chats == 'hai'){
		...
		if (mentionedJidList.includes(ownerNumber)) { /*Y'ALL CAN CUSTOM THIS WHATEVER YOU WANT!*/
			...
			urbae.reply(from, 'ngapain tag ZeusXz?\npc aja kali sabii', id).then(() => {
		...
		if (liststicker.includes(chats))
			try {
				...
				await urbae.sendImageAsSticker(from, `data:image/jpeg;base64,${getstick.toString('base64')}`, { author: "ZeusXz", pack: chats, keepScale: true })
			} catch {
				urbae.reply(from, 'Maaf, sistem error', id)
			}
		...
		if (isCmd && isMuted(chatId) && banChat() && !isBlocked && !isBanned || isOwnerB || isPrem) {
			switch (command) {
				...
				case prefix + 'tnc':
					...
					break
				case prefix + 'help':
					const bots = `Hi ${pushname}, this is ZeusXz Bot, to find out the commands menu, type *${prefix}menu* , *${prefix}p*`
...
case prefix + 'antivirtek':
			...
			break
				...
				case prefix + 'cuaca':
					...
					urbae.reply(from, mess.wait, id)
					try {
						const cuca = await axios.get(`https://zekais-api.herokuapp.com/cuaca?daerah=${cuacaq}&apikey=${zekais}`)
						const cucaq = cuca.data.data
...
case prefix + 'translate2':
        ...
        break
				...
				case prefix + 'quotemaker':
					if (args.length === 0) return urbae.reply(from, `Kirim perintah *#quotemaker2 |gagal move on|Zeus*`, id)
 ...
 break
				...
				case prefix + 'distance':
					...
					break
				case prefix + 'glitch':
					if (args.length === 0) return urbae.reply(from, `Kirim perintah *${prefix}glitch [ Teks1|Teks2 ]*, contoh *${prefix}glitch |ZeusXz|Dev Thoriq*`, id)
					try {
						urbae.reply(from, mess.wait, id)
				...
				case prefix + 'pensilart':
					if (isMedia || isImage || isQuotedImage) {
						...
						const beimgbb = await axios.get(`https://videfikri.com/api/imgbb/?urlgbr=${uploadimg}&title=ZeusXz`)
				...
				case prefix + 'stickernobg':
					if (isMedia || isImage || isQuotedImage) {
						...
						urbae.sendImageAsSticker(from, heynobg, { author: '@zeusxz', pack: 'ZeusXz', removebg: true })
							.catch((err) => {
								console.log(err)
								urbae.reply(from, err.message, id)
							})
				...
				case prefix + 'cekjodoh':
					...
					rugaapi.cekjodoh(args[0], args[1])
						.then(async (res) => {
							await urbae.sendFileFromUrl(from, `${res.thumb}`, '', `${res.text}`, id)
						})
					break
				...
				case prefix + 'memes':
					urbae.reply(from, mess.wait, id)
					urbae.sendFileFromUrl(from, `https://zekais-api.herokuapp.com/dankmemes?apikey=${zekais}`, 'img.jpg', '', id)
						.then(async (res) => {
							const kndm = res.data.result
							
							urbae.sendFileFromUrl(from, kndm, 'img.jpg', 'ichi ni san nya ari lasso', id)
								.catch(() => {
									urbae.reply(from, 'Error', id)
								})
						})
				...
				case prefix + 'quotesislam':
					axios.get(`https://urbaee-xyz.herokuapp.com/api/randomquote/muslim?apikey=ZeusXz`)
						.then(async (res) => {
...
, id)
				...
				case prefix + 'facebook':
					if (args.length == 0) return urbae.reply(from, `Kirim perintah *${prefix}fbdwn [linkfb]*`, id)
                             urbae.reply(from, '_Scrapping Metadata...._', id)
                rugaapi.facebook(args)
                .then(async(res) => {
                    await urbae.sendFileFromUrl(from, `${res.result.hd}\n\n*Setelah vidio dikirim dimohon berterimakasih*\n*Gak makasih = Block!!*\n\nBy : Zeus`, '', '', id)
                    .catch(() => {
                        urbae.reply(from, `Error...`, id)
                    })
                })
				...
				case prefix + 'exec':
					...
					oz.execCommand(q).then((res) => {
						urbae.reply(from, `> root@ZeusXz:~ # ${res}`, id)
					}).catch(err => {
						return urbae.reply(from, `> root@ZeusXz:~ # ${err}`, id)
						//console.log("os >>>", err);
					})
					break
				case prefix + 'play3':
				...
				case prefix + 'shortlink':
					...
					axios.get(`http://docs-jojo.herokuapp.com/api/shorturl-at?url=${body.slice(11)}`).then(res => {
...
*Text : ${swrt}*`
            ...
            if (args.length == 0) return urbae.reply(from, `Untuk add premium seseorang agar bisa menggunakan menu premium\n\nCaranya ketik: \n${prefix}prem add 628xx --untuk mengaktifkan\n${prefix}ban del 628xx --untuk nonaktifkan\n\ncara cepat ban banyak digrup ketik:\n${prefix}ban @tag @tag @tag`, id)
            if (args[0] == 'add') {
                prem.push(args[1]+'@c.us')
                fs.writeFileSync('./lib/database/prem.json', JSON.stringify(prem))
                urbae.reply(from, 'Success add premium number!', id)
            } else
            if (args[0] == 'del') {
                let xnxx = banned.indexOf(args[1]+'@c.us')
               prem.splice(xnxx,1)
                fs.writeFileSync('./lib/database/prem.json', JSON.stringify(prem))
                urbae.reply(from, 'Success unpremium number!', id)
            } else {
             for (let i = 0; i < mentionedJidList.length; i++) {
                prem.push(mentionedJidList[i])
                fs.writeFileSync('./lib/database/prem.json', JSON.stringify(prem))
                urbae.reply(from, 'Success premium number!', id)
                }
            }
            break
				case prefix + 'unblock':
