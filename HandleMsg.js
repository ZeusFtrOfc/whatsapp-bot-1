...
case prefix + 'yuri':
					try {
						...
						axios.get('https://zenzapi.xyz/api/anime/nsfw/yuri?apikey=${zenzapi}', { responseType: 'arrayBuffer' }).then(res => {
							urbae.sendImage(from, res.data, 'yuri.jpeg', 'Sange kok sama gambar:v');
...
case prefix + 'bjgif':
			...
			case prefix + 'memes':
					...
					urbae.sendFileFromUrl(from, res.data.result.url, 'Aowkakaka');
					});
					break
				case prefix + 'aesthetic':
					urbae.reply(from, mess.wait, id)
					axios.get(`https://api.zeks.me/api/estetikpic?apikey=${apikeyvinz}`,{ responseType: 'arrayBuffer' })
						.then(async (res) => {
							await urbae.sendImage(from, res.data, 'img.jpg', 'nihh pict estetiknya', id)
								.catch(() => {
									urbae.reply(from, 'Error', id)
								})
						})
