		...
		if (isCmd && isMuted(chatId) && banChat() && !isBlocked && !isBanned || isOwnerB || isPrem) {
			switch (command) {
				...
				case prefix + 'cuaca':
					...
					urbae.reply(from, mess.wait, id)
					try {
						const cuca = await axios.get(`https://urbaee-xyz.herokuapp.com/api/cuaca?kabupaten=${cuacaq}&apikey=ZeusXz`)
				...
				case prefix + 'doaharian':
					urbae.reply(from, mess.wait, id)
					try {
						const dataplaw = await axios.get(`https://urbaee-xyz.herokuapp.com/api/muslim/doaharian?apikey=ZeusXz`)
...
_Desc di update oleh : @${chat.groupMetadata.descOwner.replace('@c.us', '')} pada *${moment(chat.groupMetadata.descTime * 1000).format('dddd, DD MMMM YYYY')}* pukul ${moment(chat.groupMetadata.descTime * 1000).format('HH:mm:ss')}_`)
				...
				case prefix + 'translate':
					...
					await axios.get(`https://zekais-api.herokuapp.com/translate?lang=${suway2}&text=${suway1}&apikey=${zekais`).then(res => {
