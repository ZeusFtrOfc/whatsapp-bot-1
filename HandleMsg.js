...
case prefix + 'bjanime':
				...
				case prefix + 'gifhentai':
					...
					break
case prefix + 'yuri':
  if (isLimit(serial)) return urbae.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)                   
       if (!isNsfwOn) return urbae.reply(from, mess.nsfwnoton, id)
...
case prefix + 'hitungwr':
					...
					const wr1 = arg.split('|')[1]
					const wr2 = arg.split('|')[2]
					const wr3 = arg.split('|')[3]
					await axios.get('https://zenzapi.xyz/api/information/hitungwr?apikey=${zenzapi}' + '&text=' + wr1 + '&text2=' + wr2 + '&text3=' + wr3).then(res => {
